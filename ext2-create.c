...
void write_superblock(int fd) {
	...
	superblock.s_inodes_count = NUM_INODES; //128
	superblock.s_blocks_count = NUM_BLOCKS; //1024
	...
	superblock.s_first_data_block = 1; // always 1 for 1KiB sized blocks always 0 for FS w/ block sizes > /* First Data Block */
...
void write_inode_bitmap(int fd)
{
	...
	for (int i = NUM_INODES/8; i < BLOCK_SIZE; i++)
		map_value[i] = -1;
	for(int i = (LAST_INO/8) + 1; i < NUM_INODES/8; i++){
		map_value[i] = 0x0;
	}
	if(LAST_INO%8 == 7)
		map_value[LAST_INO/8] = 0x7F;
	if(LAST_INO%8 == 6)
		map_value[LAST_INO/8] = 0x3F;
	if(LAST_INO%8 == 5)
		map_value[LAST_INO/8] = 0x1F;
	if(LAST_INO%8 == 4)
		map_value[LAST_INO/8] = 0x0F;
	if(LAST_INO%8 == 3)
		map_value[LAST_INO/8] = 0x07;
	if(LAST_INO%8 == 2)
		map_value[LAST_INO/8] = 0x03;
	if(LAST_INO%8 == 1)
		map_value[LAST_INO/8] = 0x01;
	if (write(fd, map_value, BLOCK_SIZE) != BLOCK_SIZE)
	{
		errno_exit("write");
	}
...
void write_inode_table(int fd) {
	...
	strcpy(hello_sym_inode.i_block, "hello-world");
	//hello_world_inode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	...
	write_inode(fd, 2, &root_inode); //ROOT DIR INODE NUMBER IS ALWAYS 2
